...
/*  echo '<pre>';
    ...
    public function edms_document()
    {  
		$data = array(); 
		$data['method_control'] 	= '';
		Session::put('click_control',0);
		$data['Heading'] = $data['title'] = 'Add Document';
		$data['action'] 			= 'edms_document_add/'; 
		$data['button']				= 'Add';
		$data['mode']				= 'add';
		...
		$data['document_id']		= ''; 
		$data['emp_id'] 			= ''; 
		$data['national_id_new'] 	= ''; 
		$data['birth_certificate'] 	= ''; 
		$data['explanation_ans_given_date'] 	= ''; 
		$data['explanation_ans_date'] 	= ''; 
		$data['is_access'] 			= 1; 
		...
		$data['emp_document_list'] = $this->temp_edms_document();
		return   view('admin.document.emp_document_form',$data);
	/* 	echo 'ok';
		exit; */
    } 
	public function edms_document_add(Request $request)
    {	
		$data = array();  
		...
		if($data['subcat_id'] == 190){
			$image_full_name = $data['emp_id'].'_'.time().'.'.$ext;
		}else{
			$image_full_name = $data['emp_id'].'_'.$data['category_id'].'_'.$data['subcat_id'].'_'.$data['effect_date'].'_'.time().'.'.$ext;
		}
		...
		if($data['subcat_id'] == 190){
			...
			if (!empty($result)) {
				$rename_path = $_SERVER['DOCUMENT_ROOT'].'/'.$upload_path.$result;
				$file_name_ext = explode('.',$result)[1];
				$rename_path_new = $_SERVER['DOCUMENT_ROOT'].'/'.$upload_path.$emp_id.'_old.'.$file_name_ext;
				
				/* echo "<pre>";
				print_r($rename_path_new);
				exit; */
				
				if(is_readable($rename_path))
				{ 
					rename($rename_path,$rename_path_new);
				}
			}  
			$success = $images->move($upload_path,$image_full_name);
			if($success){
				DB::table('tbl_emp_photo')->where('emp_id', $emp_id)->update(['emp_photo' => $image_full_name]);
			}
		...
		if($success){
			...
			if($data['subcat_id'] == 71){
				$data['explanation_ans_date'] = $request->explanation_ans_date;
			}
			if($data['subcat_id'] == 143){
				$data['explanation_ans_given_date'] = $request->explanation_ans_given_date;
			}
			DB::table('tbl_edms_document')->insertGetId($data); 
			if($data['subcat_id'] == 40){ 
				$national_id_new = $request->national_id_new; 
				DB::table('tbl_emp_basic_info')->where('emp_id', $emp_id)->update(['national_id_new' => $national_id_new]);
			}
			if($data['subcat_id'] == 154){ 
				$birth_certificate = $request->birth_certificate; 
				DB::table('tbl_emp_basic_info')->where('emp_id', $emp_id)->update(['birth_certificate' => $birth_certificate]);
			}
		...
		Session::put('click_control',1);
		Session::put('message','Data Add Successfully.');
		return Redirect::to("/edms_document_empid/$emp_id");
    }
	public function edms_document_empid($emp_id)
    {   
		$data = array(); 
		Session::put('click_control',1); 
		$data['method_control'] 	= '';
	...
	public function temp_edms_document()
	{
		$user_id 					= Session::get('admin_id');
		 $emp_document_list = DB::table('tbl_edms_document as docu') 
									->leftJoin("tbl_emp_basic_info as emp",function($join){
											$join->on("emp.emp_id","=","docu.emp_id");
										}) 
									->leftJoin('tbl_edms_category as cat', 'docu.category_id', '=', 'cat.category_id')
									->leftJoin('tbl_edms_subcategory as subcat', 'docu.subcat_id', '=', 'subcat.subcat_id')  
									->where('docu.is_cancel',0)
									->where('docu.is_save',0)
									->where('docu.user_id',$user_id)
									->select('docu.*','emp.emp_name_eng as  emp_name','cat.category_name','subcat.subcategory_name')
									->orderby('docu.document_id','desc')
									->orderby('docu.effect_date','desc')
									->limit(300)
									->get();
			return $emp_document_list;
	} 
	public function roll_wise_edms_permission($br_code){
		$is_access = 0;
		$access_label 	= Session::get('admin_access_label');
		if($access_label == 33 || $access_label == 34 || $access_label == 35 || $access_label == 38 || $access_label == 1  ) {
				$is_access = 1;
		}else{
			if($br_code != 9999){
				$is_access = 1;	
		...
		$assign_designation = ''; 
		 ...
		 if(!empty($emp_info)){
			 ...
			 if(!empty($assign_designation)){
			  ...
			  $is_access =  $this->roll_wise_edms_permission($br_code);	
			echo json_encode(
				array(
    ...
    { 
		...
		$data['action'] 			= "edms_document_update";  
		$data['button']				= 'Update';
		$data['mode']				= 'edit';
		$data['required']			= '';
		Session::put('click_control',0);
		...
		$data['method_control'] ="<input type='hidden' name='_method' value='PUT' />"; 
			...
			$current_date = date('Y-m-d');  
				...
				$emp_info = DB::table('tbl_master_tra as m') 
									...
									->where('m.sarok_no', '=', $max_sarok->sarok_no)
									->select('emp.emp_id','emp.national_id_new','emp.birth_certificate','emp.emp_name_eng as emp_name','b.br_code','d.designation_name','d.designation_code','b.branch_name','dp.department_name')
									->first(); 
				$assign_emp = DB::table('tbl_emp_assign as ea')
									->where('ea.emp_id',$emp_id)
									->where('ea.status', '!=', 0)
			...
			$data['national_id_new']	= $emp_info->national_id_new;
			$data['birth_certificate']	= $emp_info->birth_certificate;
		...
		$data['explanation_ans_date']= $document_by_id->explanation_ans_date; 
		$data['explanation_ans_given_date']= $document_by_id->explanation_ans_given_date; 
		$data['category_list']  	= DB::table('tbl_edms_category') 
											->where('status',1)
											->orderby('category_name','asc')
											->get();
		...
		$data['emp_document_list'] = $this->temp_edms_document();
		$data['is_access'] = $this->roll_wise_edms_permission($br_code);
		return   view('admin.document.emp_document_form',$data);
    }
	public function edms_save_flag()
	{
		$data = array();
		$data['is_save'] = 1;
		$user_id 					= Session::get('admin_id');
		DB::table('tbl_edms_document')
				...
				->update($data);
		Session::put('message','Data Saved Successfully.');
		return Redirect::to('/edms');		
	}
    public function edms_document_update(Request $request)
    {
		$data = array();  
		$temp_row_count = $request->temp_row_count;
        ...
        $data['effect_date'] 			= $request->effect_date;
        $hidden_subcat_id				= $request->hidden_subcat_id;
		   ...
		   if($hidden_category_id == 13){
			...
			if($hidden_subcat_id == 190){
				if($hidden_document_name){
					$file_name = "public/employee/$hidden_document_name";
		...
		/* if($temp_row_count > 1){
			return Redirect::to('/edms_document');
		}else{
			return Redirect::to('/edms');
		} */
