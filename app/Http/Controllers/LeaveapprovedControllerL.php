...
class LeaveapprovedController extends Controller
{
	...
	public function approvedleave($id,$f_year_id,$emp_id,$emp_type)
    { 
		$datahis = array();
		$data = array();
		$br_code = 9999;
		$form_date 				= date('Y-m-d'); 
		$designation			= array(1,26,29,38,47,99,166,48,194,111,145,144,167,162,147,209,211,225,263,257);
		...
		$datahis['db_id'] 	= $id;
		 if($emp_type == 1){
			 	...
			 	$emp_leave_his = DB::table('tbl_leave_history')  
					 ...
					 ->where('tbl_leave_history.id',$id)  
                     ->select('tbl_leave_history.*','nid.emp_name','nid.joining_date as org_join_date','b.branch_name','d.designation_name')
					 ->first(); 
		 }
        ...
        $datahis['emp_type'] 			= $emp_leave_his->emp_type; 
	...
	public function search_emp_info(Request $request)
    {
		...
		if($emp_type == 1){
			...
			$datahis['employee_his']  = DB::table('tbl_emp_non_id as nid')  
										// ->leftJoin('tbl_nonid_official_info as oinf', 'oinf.emp_id', '=', 'nid.emp_id') 
										 ...
										 ->where('nid.sacmo_id',$employee_id)  
										 ->where('nid.emp_type_code',$emp_type)  
										 ->select('nid.sacmo_id as emp_id','nid.emp_name','nid.joining_date','b.br_code','d.designation_code','b.branch_name','d.designation_name')
										 ->first(); 
			/*  echo '<pre>';
...
exit;		 	 */		 
	...
	public function approved_leave_delete($id,$f_year_id,$emp_id)
    {
		...
		$emp_type 				= $leave_history->emp_type;
	...
	public function leave_date_exist_edit($from_date,$to_date,$emp_type,$emp_id,$db_id){
