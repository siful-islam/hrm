...
class LeavehrmheadController extends Controller
{
    ...
    public function recommend_leave_hrhead($id,$emp_id)
    {
		$datahis = array();
		...
		}else if($datahis['supervisor_type'] == 1) { 
				$datahis['button'] 				= "Approved"; 
				$datahis['recommend_type'] 		= 2; 
		}
		}else{
				$datahis['supervisor_type']     =1;
				$datahis['button'] 				= "Approved"; 
				$datahis['recommend_type'] 		= 2;
		}
		...
		$datahis['id'] 					= $id;  
		$datahis['action']				= "/approved_by_hr_head/$id"; 
		...
		return view('admin.leave.emp_recommend_form_hrhead',$datahis);
    }public function recommend_leave_view_hrhead($id,$emp_id)
    {
		$datahis = array();
		$datahis['method_control'] ="<input type='hidden' name='_method' value='PUT' />"; 
		$datahis['leave_type'] 	= DB::table('tbl_leave_type')->get();
    ...
    public function approved_by_hr_head(Request $request,$id)
    {
		$data = array();
        $id = $request->id;  
		...
		$data['hrhd_action'] = 2;
		$data['hrhd_emp_id'] = Session::get('emp_id');
		$data['hrhd_action_date'] = date('Y-m-d');  
        $data['stage'] = 3;
	...
	public function approved_by_sup_reject_hrhead($id)
    {
		$data = array();
		$data['hrhd_action'] = 4;
		$data['stage'] = 3;
	...
	public function approve_leave_byhrm($id)
    {
		...
		$hrhd_action 				= $leave_appli_info->hrhd_action;
		$super_action 				= $leave_appli_info->super_action;
		
		if($hrhd_action == 2){
			$approved_id 				= $leave_appli_info->hrhd_emp_id;
		}else if($super_action == 2){
