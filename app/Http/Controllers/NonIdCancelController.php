...
use App\Models\NonIdCancel;
use Session;
use Illuminate\Support\Facades\Redirect;
//session_start();

class NonIdCancelController extends Controller
{
    ...
    {        	
		...
		$data['results'] = DB::table('tbl_emp_non_id_cancel')
		
							->leftjoin('tbl_emp_non_id',function($join) use($current_date){
		...
		return view('admin.employee.manage_non_id_cancel',$data);					
    }
	...
	public function create()
    {
		...
		$data['mode'] 				= '';
		//
		$data['action'] 			= '/non-cancel';
		$data['method'] 			= 'POST';
		$data['method_control'] 	= ''; 
		$data['Heading'] 			= 'Add Contractual Cancel';
		$data['button_text'] 		= 'Save';			
		//
		$data['all_emp_type'] 			= DB::table('tbl_emp_type')->where('id','>',1)->where('status',1)->get();
		return view('admin.employee.non_id_cancel_form',$data);	
    }	
	
	
	public function get_emp_info($emp_id,$emp_type) 
	{
		$data = array();
		/*  if($emp_type1 == 1 ){
				$emp_type = 'non_id';
	...
	public function edit($id)
	{
		...
		$data['action'] 			= "/non-cancel/$id";
		$data['method'] 			= 'POST';
		$data['method_control'] 	= "<input type='hidden' name='_method' value='PUT' />"; 		
		$data['Heading'] 			= 'Edit Contractual Cancel';
    ...
    {       
        ...
        {            
            $results = NonIdCancel::leftjoin('tbl_emp_non_id', 'tbl_emp_non_id.emp_id', '=', 'tbl_emp_non_id_cancel.emp_id' )
							->leftjoin('tbl_branch', 'tbl_branch.br_code', '=', 'tbl_emp_non_id_cancel.br_code' )
							->leftjoin('tbl_emp_type as et',function($join){
										$join->on('et.id', "=","tbl_emp_non_id_cancel.emp_type_code"); 
									})	
		...
		else 
		{
            ...
            $results =  NonIdCancel::leftjoin('tbl_emp_non_id', 'tbl_emp_non_id.emp_id', '=', 'tbl_emp_non_id_cancel.emp_id' )
							...
							->orderBy('tbl_emp_non_id_cancel.emp_code', 'desc')
                            ->get();
            $totalFiltered = NonIdCancel::where('tbl_emp_non_id_cancel.emp_code','LIKE', "%{$search}%")->orWhere('tbl_emp_non_id_cancel.emp_type_code', 'LIKE',"%{$search}%")
                            ->count();
        }
        ...
        if(!empty($results))
        {
            ...
            foreach ($results as $result)
            {
				...
				$nestedData['options'] 			= '<a class="btn btn-sm btn-success btn-xs" title="Edit" href="non-cancel/'.$result->id.'/edit"><i class="glyphicon glyphicon-pencil"></i> Edit</a>';
