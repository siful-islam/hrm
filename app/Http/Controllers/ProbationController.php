...
class ProbationController extends Controller
{
    ...
    {        	
		//$data['probation_info'] = Probation::join('tbl_appointment_info', 'tbl_probation.emp_id', '=', 'tbl_appointment_info.emp_id' )
							//->orderBy('tbl_probation.emp_id', 'desc')
    ...
    public function create()
    {
		...
		$data['report_to'] 				= '' ;
		$data['permanent_date'] 	= date('Y-m-d',strtotime($data['effect_date'] . "+6 month"));
		$data['status'] 				= 1;
		//
    ...
    public function edit($id)
    {
		...
		$data['permanent_date'] 		= $probation_info->permanent_date;
    ...
    {       
        ...
        if(!empty($probations))
        {
            ...
            foreach ($probations as $v_probations)
            {
				...
				$nestedData['options'] 		= '<a class="btn btn-sm btn-danger btn-xs" title="Edit" href="#"><i class="glyphicon glyphicon-lock"></i></a>';				
				$data[] = $nestedData;
            }
        }
        $json_data = array(
