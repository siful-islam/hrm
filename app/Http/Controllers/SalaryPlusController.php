...
class SalaryPlusController extends Controller
{
    ...
    {        	
		$data['items'] = Salary_plus::join('tbl_designation as d1', 'd1.designation_code', '=', 'tbl_salary_plus.designation_for','left')
							->join('tbl_plus_items as item', 'item.item_id', '=', 'tbl_salary_plus.item_name','left')
							->join('tbl_department as dept', 'dept.id', '=', 'tbl_salary_plus.emp_department','left')
							->join('tbl_grade_new as grade', 'grade.grade_id', '=', 'tbl_salary_plus.emp_grade','left')
							->select('tbl_salary_plus.*','d1.designation_name','item.items_name','dept.department_name','grade.grade_name')
							->get(); 
							
		return view('admin.settings.manage_salary_plus',$data);					
    }
	public function create()
    {
		...
		$data['status'] 			= 1;
		$data['active_from'] 		= '2018-07-01';
		$data['active_upto'] 		= '2020-06-30';
		//
		$data['action'] 			= '/salary-plus';
		$data['method'] 			= 'POST';
		$data['method_control'] 	= ''; 		
		$data['Heading'] 			= 'Add Salary Plus Item';
		$data['button_text'] 		= 'Save';
		$data['plus_items'] 		= DB::table('tbl_plus_items')->where('status',1)->get();	
		...
		return view('admin.settings.plus_form', $data);
    } 
	
	public function edit($id)
    {
       
	    $info = Salary_plus:: where('id', $id)->first();
		$action_id = 2;
		$get_permission 				= $this->cheeck_action_permission($action_id);
		if($get_permission == false)
		{
		...
		//
		$data['action'] 			= "/salary-plus/$id";
		$data['method'] 			= 'POST';
		$data['method_control'] 	= "<input type='hidden' name='_method' value='PUT' />";	
		$data['Heading'] 			= 'Edit Salary Plus Item';
	...
	public function store(Request $request)
    {
		...
		$status = DB::table('tbl_salary_plus')->insertGetId($data);
		if($status)
		{	
		...
		return Redirect::to('/salary-plus');
    }
    ...
    public function update(Request $request, $id)
    {
		...
		$status = DB::table('tbl_salary_plus')
            ->where('id', $id)
            ->update($data);
