...
class ShoesizeController extends Controller
{
	...
	public function shoesize()
    {
		...
		//$branch_code 			= 1;
		$data['emp_list'] = DB::table('tbl_shoesize') 
									->leftJoin("tbl_emp_basic_info as emp",function($join){
											$join->on("emp.emp_id","=","tbl_shoesize.emp_id");
									})
									 ->leftJoin('tbl_branch as b', 'tbl_shoesize.branch_code', '=', 'b.br_code') 
									 ->leftJoin('tbl_designation as d', 'tbl_shoesize.designation_code', '=', 'd.designation_code') 
									->where('tbl_shoesize.branch_code',$branch_code) 
									->select('tbl_shoesize.*','emp.emp_name_eng as emp_name','b.branch_name','d.designation_name','emp.gender')
									->get();
		...
		print_r($tot_value );
		exit;  */
		return view('admin.requisition.manage_shoesize',$data);
    }
	public function shoesize_create()
    {
		$data = array();
		...
		if (!empty($all_result1)) {
			foreach ($all_result1 as $result) {
				...
				$data_result = DB::table('tbl_master_tra as m')
							...
							->select('e.emp_name_eng','e.org_join_date','e.permanent_add','m.br_join_date','m.br_code','m.designation_code','d.designation_name','b.branch_name','ea.incharge_as','e.gender')
							->first();
				//print_r ($data_result);
				//// employee assign ////
		...
		return view('admin.requisition.shoesize_form',$data);
    } 
	public function shoesize_update()
    {
		$data = array();
		$form_date 		= date("Y-m-d");
		$branch_code 			= Session::get('branch_code');
		...
		return view('admin.requisition.shoesize_form_update',$data);
    } 
	public function insert_shoesize(Request $request)
    {
		$data 				= array();
		$data1 				= array();
		...
		exit; */ 
			...
			for($i = 1; $i < $tot_row; $i++ ){  
				...
				DB::table('tbl_shoesize') 
					->insert($data);   	
			}
	 
		//exit;
		return Redirect::to('/shoesize'); 
    }
	public function update_shoesize(Request $request)
    {
		$data 				= array();
		$data1 				= array(); 
		$data['user_code']	= Session::get('admin_id'); 
	...
	public function shoe_size_rprt()
    {
		$data = array();
		...
		return view('admin.requisition.shoe_size_report',$data);
		 
		
    } 
	public function shoe_size_rprt_post(Request $request)
    {
		$data = array();
	 ...
	 */
	 if(($area_code == 'all') || ($area_code == 'all_a')){
		 $emp_list = DB::table('tbl_shoesize') 
					->select('tbl_shoesize.*')
					->get();
			...
			exit;   */
				 if(!empty($max_sarok)){
					 ...
					 $data_result = DB::table('tbl_master_tra as m')
								...
								->where('m.sarok_no', '=', $max_sarok->sarok_no)
								->select('m.br_code','b.branch_name','e.emp_name_eng as emp_name','d.designation_name','ar.area_code','e.gender')
								->first();
				 ...
				 if (!empty($all_result)) {
					foreach ($all_result as $result) { 
						...
						$emp_info = DB::table('tbl_master_tra as m') 
									...
									->leftjoin('tbl_shoesize as dl',function($join){
									...
									->leftJoin('tbl_area as ar', 'b.area_code', '=', 'ar.area_code')
									->where('m.sarok_no', '=', $max_sarok->sarok_no)
									->select('emp.emp_id','dl.emp_id as emp_id1','emp.emp_name_eng as emp_name','b.br_code','d.designation_name','d.designation_code','b.branch_name','dl.size','ar.area_code','emp.gender')
									->first(); 
								...
								$all_result1 = array_unique(array_merge($all_result,$assign_branch), SORT_REGULAR);
									if (!empty($all_result1)) { 
										...
										$emp_info = DB::table('tbl_master_tra as m') 
															...
															->leftJoin('tbl_area as ar', 'b.area_code', '=', 'ar.area_code')
															->where('m.sarok_no', '=', $max_sarok->sarok_no) 
															->select('emp.emp_id','emp.gender','dl.emp_id as emp_id1','dl.size', 'emp.emp_name_eng as emp_name','b.br_code','d.designation_name','d.designation_code','b.branch_name','ar.area_code')
															->first(); 
													/* echo '<pre>';
													print_r($emp_info); */
												 if(!empty($emp_info->emp_id1)){
