...
class TransfersController extends Controller
{
    ...
    {       
        ...
        {            
            $transfers = Transfer::join('tbl_emp_basic_info', 'tbl_transfer.emp_id', '=', 'tbl_emp_basic_info.emp_id' )
							...
							->select('tbl_transfer.id','tbl_transfer.emp_id','tbl_transfer.sarok_no','tbl_transfer.letter_date','tbl_transfer.effect_date','tbl_transfer.status','tbl_emp_basic_info.emp_name_eng','tbl_designation.designation_name','tbl_branch.branch_name','tbl_transfer.br_joined_date')
							->get();
        }
        else {
            ...
            $totalFiltered = Transfer::join('tbl_emp_basic_info', 'tbl_transfer.emp_id', '=', 'tbl_emp_basic_info.emp_id' )
							->where('tbl_transfer.emp_id', $search)
                             ->count();
        }
        ...
        if(!empty($transfers))
        {
            ...
            foreach ($transfers as $v_transfers)
            {
                ...
                $nestedData['emp_name'] 	= $v_transfers->emp_name_eng;
				...
				$nestedData['status'] 		= $v_transfers->br_joined_date;
				$nestedData['options'] 		= '<a class="btn btn-sm btn-success btn-xs" title="Edit" href="transfer/'.$v_transfers->id.'/edit"><i class="fa fa-eye"></i> View</a> 
				 <a class="btn btn-sm btn-danger btn-xs" title="Lock" href="#"><i class="glyphicon glyphicon-lock"></i></a>';		
    ...
    public function create()
    {
		...
		//
		$data['branches'] 		    	= DB::table('tbl_branch')->where('status',1)->orderBy('tbl_branch.branch_name', 'asc')->get();
    ...
    public function edit($id)
    {
		...
		$master_info = DB::table('tbl_master_tra')->where('sarok_no', $transfer_info->sarok_no)->first();
		$emp_id 						= $transfer_info->emp_id;
		$data['emp_id'] 				= $transfer_info->emp_id;
		$data['letter_date'] 			= $transfer_info->letter_date;
		$data['effect_date'] 			= $transfer_info->effect_date;
		$data['br_joined_date'] 		= $transfer_info->br_joined_date;
		$data['grade_effect_date'] 		= $transfer_info->grade_effect_date;
		$data['sarok_no'] 				= $transfer_info->sarok_no;
		$data['br_code'] 				= $transfer_info->br_code;
		$data['salary_br_code'] 		= $master_info->salary_br_code;
	...
	public function get_transfer_info($emp_id , $q_type)
	{
		$results = DB::table('tbl_transfer')
						...
						->select('tbl_transfer.id','tbl_transfer.br_joined_date','tbl_branch.branch_name')
						->where('emp_id',$emp_id) 
		...
		foreach($results as $result){
			...
			echo '<tr>';
			echo "<td>$i</td>";
			echo "<td>$result->br_joined_date</td>";
			...
			echo '</tr>';
			$next_day = $result->br_joined_date;
			$i++;
		}		
