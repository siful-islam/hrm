...
<head>
	...
	<link rel="shortcut icon" href="{{asset(Session::get('favicon'))}}">
	...
	<!-- Gritter -->
	<link rel="stylesheet" type="text/css" href="{{asset('public/admin_asset/bower_components/gritter/css/jquery.gritter.css')}}" />
	<link rel="stylesheet" href="{{asset('public/admin_asset/plugins/jQuery/jquery-ui.css')}}">

	<!--<link rel="stylesheet" type="text/css" href="{{asset('public/admin_asset/bower_components/select2/dist/css/select2.min.css')}}" />-->

	<!-- Theme style -->
	<link rel="stylesheet" href="{{asset('public/admin_asset/dist/css/AdminLTE.min.css')}}">
	<link rel="stylesheet" href="{{asset('public/admin_asset/dist/css/skins/skin-blue.min.css')}}">		   
	<!-- jQuery 3 -->
...
<div class="wrapper">
    ...
    <header class="main-header">
		...
		<a href="#" class="logo">
			...
			<span class="logo-mini"><img src="{{asset(Session::get('org_logo'))}}" width="32"></span>
			<!-- logo for regular state and mobile devices -->
			<span class="logo-lg"><img src="{{asset(Session::get('org_logo'))}}" width="32"> <b>{{Session::get('org_short_name')}}</b>HRM</span>
		</a>
		<!-- Header Navbar: style can be found in header.less -->
		<nav class="navbar navbar-fixed-top">
		 <a href="http://hrm.microfineye.com/public/CDIP_Manual.pdf" target="_BLANK"><button type="button" class="btn btn-primary"><strong>CDIP Manual</strong></button></a>
		  <!-- Sidebar toggle button-->
			...
			<ul class="nav navbar-nav">
			  ...
			  <li class="dropdown messages-menu">
					 <a href="{{URL::to('/br_download')}}"class="pull-right" style="color:white;">Download</a>
				</li>-->
			<!--	<li class="dropdown messages-menu">
				...
				<li class="dropdown notifications-menu">
					...
					<ul class="dropdown-menu">
						...
						<li class="footer"><a href="{{URL::to('/br_transfer')}}">View all</a></li>
					</ul>
				</li>
				<?php } ?>
				...
				<li class="dropdown user user-menu">
					<a href="#" class="dropdown-toggle" data-toggle="dropdown">
						<img src="{{asset(Session::get('admin_photo'))}}" class="user-image" alt="User Image">
						<span class="hidden-xs"><?php  
							$user_type = Session::get('user_type');
							if($user_type == 3){ ?> 
								 {{Session::get('zone_name')}} Zone
							<?php }else if($user_type == 4){ ?>
								{{Session::get('area_name')}} Area
							<?php }else{ ?>
					...
					<ul class="dropdown-menu">
						...
						<li class="user-header">
							<img src="{{asset(Session::get('admin_photo'))}}" class="img-circle" alt="User Image">
							<p><?php  
			...
			<ul class="sidebar-menu" data-widget="tree" id="left_menu"> 
				...
				<?php if(Session::get('branch_code') == 9999) { ?>
					...
					$visit_pending = DB::table('supervisor_mapping_ho as staff')
						...
						if($is_supervisor)
						{
							//$notification_counts = notification_count();	
							$notification_counts = 0;	
						}else{
							$notification_counts = 0;	
						}							
						$notification_number = 	count($leave_pending) + count($visit_pending) + $notification_counts;	
					if(!empty($is_supervisor)) {
						Session::put('supervisors_type',$is_supervisor->supervisors_type);
					}		
					?>
				<li class="treeview" id="MainGroupSelf_Care">
				  ...
				  <ul class="treeview-menu">
					...
					<li id="My_Profile"><a href="{{ URL::to('/profile')}}"><i class="fa fa-dashboard"></i> My Dashboard</a></li>
					<li id="My_Basic"><a href="{{ URL::to('/basic_info')}}"><i class="fa fa-user"></i> My Profile</a></li>
					<li id="Pay_Slip"><a href="{{ URL::to('/pay_slips')}}"><i class="fa fa-money"></i> Pay Slip</a></li>
					<li id="My_Benefits"><a href="{{ URL::to('/my_benefit')}}"><i class="fa fa-bar-chart"></i> My Benefit</a></li>
					<li id="Leave_and_Visit"><a href="{{ URL::to('/leave_visit')}}"><i class="fa fa-calendar-o"></i> Leave & Visit</a></li>
					<li id="My_Loan"><a href="{{ URL::to('/my_loan')}}"><i class="fa fa-money"></i> My Loan</a></li>
					<!--<li id="My_Attendence"><a href="{{ URL::to('/attendence')}}"><i class="fa fa-calendar"></i> Attendence</a></li>-->
					<li id="Personal_File"><a href="{{ URL::to('/my_documents')}}"><i class="fa fa-files-o"></i> Personal File</a></li>
					<?php } ?>
					<?php if($is_supervisor) {  ?>
					<li id="Leave_Approve">
					<a href="{{ URL::to('/leave_approved')}}"><i class="fa fa-check-circle"></i> Leave Approve <span class="pull-right-container">
					<span class="label label-warning pull-right" id="leave_approve_pending"><?php echo count($leave_pending); ?></span>
					</span></a>
					</li>
					<li id="Visit_Approval"><a href="{{ URL::to('/movement_approve')}}"><i class="fa fa-check-circle"></i> Visit Approve<span class="pull-right-container">
					<span class="label label-danger pull-right" id="visit_approve_pending"><?php echo count($visit_pending); ?></span>
					</span> </a></li>
				    <li id="loan_recomendation">
						<a href="{{ URL::to('/pending_approval')}}"><i class="fa fa-check-circle"></i> Loan Recommendation <span class="pull-right-container">
					</span></a>
					</li>
					<li id="group-up-activities"><a href="{{ URL::to('/upcomming-notification')}}"><i class="fa fa-check-circle"></i> Upcoming Activities<span class="pull-right-container">
					<span class="label label-danger pull-right" id="up_activities"><?php echo $notification_counts; ?></span>
					</span> </a></li>
					<?php } ?>
				...
				<li class="treeview" id="MainGroupPms">
				  ...
				  <ul class="treeview-menu">
					<li id="own_pms"><a href="{{URL::to('/pms')}}"><i class="fa fa-circle-o"></i>Own PMS</a></li>
					<?php if(!empty($is_supervisor)) { ?>
					<li id="team_pms"><a href="{{URL::to('/pms-supervisor')}}"><i class="fa fa-circle-o"></i><span>Team PMS</span></a></li>
					<?php } ?>
					<?php if($emp_id == 2692 || $emp_id == 4646 || $emp_id == 4188 || $emp_id == 4664 || $emp_id == 4891 || $emp_id == 200180) { ?>
					<li id="assessment_report"><a href="{{URL::to('/pms-assessment-report')}}"><i class="fa fa-user"></i><span>Assessment Report</span></a></li>
					<li id="report_pms"><a href="{{URL::to('/pms-report')}}"><i class="fa fa-circle-o"></i><span>PMS Status Report</span></a></li>					
					<li id="own_pms"><a href="{{ URL::to('/pms-assessment')}}"><i class="fa fa-user"></i>All PMS Detail</a></li>
					<?php } ?>
				  </ul>
				</li>
					<?php 
					...
					if($emp_id == 1230){?>
							<li class="treeview" id="MainGroupperedms">
							  ...
							  <ul class="treeview-menu">
								...
								<li id="personal_edms"><a href="{{ URL::to('/per_document')}}"><i class="fa fa-user"></i> Personal Documents</a></li> 
							  </ul>
							</li>
					<?php }?>
				...
				if(count($navbar_group)<1  && $head_department ){  ?>
					<li class="treeview" id="MainGrouphead_edms">
						  ...
						  <ul class="treeview-menu">
							<li id="My_Profile"><a href="{{ URL::to('/edms_document_hlist_index')}}"><i class="fa fa-book"></i> Document list</a></li>
				...
				<?php foreach($navbar_group as $v_navbar_group) { ?>
					...
					<li class="treeview" id="MainGroup<?php echo preg_replace('/\s+/', '_',  $v_navbar_group->nav_group_name); ?>">
						...
						<ul class="treeview-menu">
						@foreach ($navbar as $v_navbar)
						@if ($v_navbar_group->nav_group_id ==  $v_navbar->nav_group_id)
						<li class="" id="<?php echo preg_replace('/\s+/', '_',  $v_navbar->nav_name); ?>"><a href="{{URL::to($v_navbar->nav_action)}}"><i class="fa fa-circle-o"></i> {{ $v_navbar->nav_name }}</a></li>
						@endif
						<?php 
...
<script src="{{asset('public/admin_asset/plugins/datatables/dataTables.bootstrap.min.js')}}"></script>
<script src="{{asset('public/admin_asset/plugins/jQuery/jquery-ui.min.js')}}"></script>	
