...
<body>
	...
	<table width="100%">
		...
		<tr>
			...
			<td>:</td>
			<td>{{ $mail_data['emp_name'] }}</td>
		</tr>
		<tr>
			<td>Employee ID </td>
			<td>:</td>
			<td>{{ $mail_data['emp_id'] }}</td>
		</tr>
		<tr>
			<td>Leave Period</td>
			<td>:</td>
			<td>{{ date('d-m-Y', strtotime($mail_data['leave_from'])) }} to {{ date('d-m-Y', strtotime($mail_data['leave_to'])) }}</td>
		</tr>
		<tr>
			<td>Duration</td>
			<td>:</td>
			<td>{{ $mail_data['no_of_days'] }} <?php if($mail_data['no_of_days'] < 2 ) { echo 'day'; }else { echo 'days'; }?></td>
		</tr>
		<tr>
			<td>Purpose </td>
			<td>:</td>
			<td valign="top">{{ $mail_data['remarks'] }}</td>
		</tr>
		<tr>
			<td>Leave Source </td>
			<td>:</td>
			<td valign="top">{{ $mail_data['leave_type_name'] }}</td>
		</tr>
		<?php
		...
		<tr>
			...
			<td>:</td>
			<td valign="top">{{ $application_type }}</td>
		</tr>
		<?php
		if($mail_data['modify_cancel'] > 0) { ?>
		<tr>
			...
			<td>:</td>
			<td valign="top">{{ $mail_data['modify_remarks'] }}</td>
		</tr>
		<?php } ?>
	</table>
	<br>
    ...
    <div>
        <a id="accept" href="{{ URL::to('/leave_mail_action' . '/' . $mail_data['application_id'] . '/' . $mail_data['supervisors_emp_id'] . '/1' . '/1') }}"
            target="_blanck">Approve</a>
        &nbsp;&nbsp;&nbsp;&nbsp; |
        &nbsp;&nbsp;&nbsp;&nbsp;
        <a id="reject" href="{{ URL::to('/leave_mail_action' . '/' . $mail_data['application_id'] . '/' . $mail_data['supervisors_emp_id'] . '/2' . '/1') }}"
