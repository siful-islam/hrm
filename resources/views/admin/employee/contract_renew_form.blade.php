...
@section('title', 'Contractual | Renew Info')
			...
			<form id="form" class="form-horizontal" <?php if(empty($mode)){?>  onsubmit="return validateForm()" <?php } ?> name="employee_nonid" action="{{URL::to($action)}}" method="post" enctype="multipart/form-data">
				...
				<div class="box-body col-md-8">	
					...
					<div class="form-group">
						...
						<div class="col-sm-3">
							<input type="text" class="form-control" <?php if($mode == 'edit'){?> readonly <?php } ?> id="emp_id" name="emp_id" value="{{$emp_id}}" onChange="get_employee_info();" required> 
						</div>
					...
					<div class="form-group">
					 <label class="control-label col-md-3">Contract Renew Date: <span style="color:red;">*</span> </label>
						...
						<div class="col-md-3">
							<input type="date" name="c_end_date" <?php if(empty($c_end_date)){ ?>  readonly <?php }  ?> id="c_end_date" value="{{$c_end_date}}" class="form-control">
	...
	function validateForm()
		{
			...
			//alert(emp_id);
			$.ajax({
				url : "{{ url::to('check_contract_effect_date') }}"+"/"+ emp_id+"/"+ effect_date,
				type: "GET",
				async: false,
	...
	function get_employee_info()
		{
			...
			$.ajax({
				url : "{{URL::to('get_nonemployee_contract_info')}}"+"/"+ emp_id,
				type: "GET",
				dataType: "JSON",
				success: function(data)
				{
