...
@section('title', 'Contractual | Information')
							...
							<?php 
								...
								foreach($results as $v_result){ ?>
									...
									<?php if($v_result->cancel_date == ''){ ?>
										<td><a class="btn btn-sm btn-success btn-xs" onclick="is_contract_check_edit('<?php echo $v_result->id;?>','<?php echo $v_result->emp_id;?>');"><i class="glyphicon glyphicon-pencil"></i> Edit</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-sm btn-info btn-xs" title="view" href="{{URL::to('view-non-id/'.$v_result->id)}}"><i class="fa fa-eye"></i> View</a></td> 
									<?php }else{ ?> 
											<td><a class="btn btn-sm btn-info btn-xs" title="view" href="{{URL::to('view-non-id/'.$v_result->id)}}"><i class="fa fa-eye"></i> View</a></td> 
									<?php } ?>
	...
	function is_contract_check_edit(id,emp_id)
		{
			...
			$.ajax({
				url : "{{ URL::to('is_contract_check_edit')}}"+"/"+id+"/"+emp_id, 
				type: "GET",
				dataType: 'json',
				success: function(data)
				{
						...
						return; */
						 if((data['is_exist_official'] == 1) || (data['is_exist_salary'] == 1 )){
							window.location.href = "{{ URL::to('non_id_info_edit') }}"+"/"+id;
						}else{ 
							window.location.href = "{{ URL::to('non-appoinment') }}"+"/"+id+"/edit";
						} 
				}
			}); 
		}
