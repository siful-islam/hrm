...
.required {
			...
			<form id="form" class="form-horizontal" action="{{URL::to($action)}}" method="{{$method}}" enctype="multipart/form-data">
				...
				<div class="box-body col-md-9">
					...
					<div class="form-group">
						...
						<div class="col-sm-3 {{ $errors->has('emp_code') ? ' has-error' : '' }}">
							<input type="text" class="form-control" id="emp_code" <?php if($mode == 'edit'){?> readonly <?php } ?> name="emp_code" value="{{$emp_code}}" onChange="get_employee_info();" required>
	...
	function get_employee_info1(){
		...
		function get_employee_info()
		{
			...
			//return;
			 $.ajax({ 
				url : "{{URL::to('get-nonid-info')}}"+"/"+ emp_code +"/"+emp_type,
				type: "GET",
				dataType: "JSON",
