...
.required {
	...
	<section class="content-header">
		<h1>Employee <small>Probation</small></h1>
		<ol class="breadcrumb">
	...
	<section class="content">
		<div class="box box-info">
			...
			<form id="form" class="form-horizontal" action="{{URL::to($action)}}" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" id="action_table" name="action_table" value="{{$action_table}}">
				<input type="hidden" id="action_controller" name="action_controller" value="{{$action_controller}}">
				<input type="hidden" id="transection_type" name="transection_type" value="{{$transection_type}}">
				<input type="hidden" id="br_joined_date" name="br_joined_date" value="{{$br_joined_date}}">
				<input type="hidden" id="is_permanent" name="is_permanent" value="{{$is_permanent}}">
				<div class="box-body col-md-9">
					...
					<div class="form-group">
						...
						<div class="col-sm-3">
							<select class="form-control" style="width: 100%;" id="department_code" name="department_code" required>						
								...
								<option value="{{$v_departments->id}}">{{$v_departments->department_name}}</option>
								@endforeach
							</select>
						</div>
					...
					<div class="form-group">
						...
						<label for="report_to" class="col-sm-2 control-label">Reported To <span class="required">*</span> </label>
						<div class="col-sm-3">
							<input type="text" class="form-control" name="report_to" id="report_to" value="{{$report_to}}">
						</div>
						<label for="zone_code" class="col-sm-2 control-label"> Probation Time <span class="required">*</span> </label>
						<div class="col-sm-2">
							<input type="number" class="form-control" name="probation_time" id="probation_time" value="{{$probation_time}}">
						</div>
						<div class="col-sm-1">
					...
					<div class="form-group">
						<label for="zone_code" class="col-sm-2 control-label"> Permanent Date <span class="required">*</span> </label>
						<div class="col-sm-3">
							<input type="date" class="form-control" name="permanent_date" id="permanent_date" value="{{$permanent_date}}">
							<input type="hidden" class="form-control" name="next_increment_date" id="next_increment_date" value="{{$permanent_date}}">
						</div>
	...
	<script>	
		...
		function set_next_increment_date()
		{
			var effect_date = document.getElementById("effect_date").value;
			var probation_time = parseFloat(document.getElementById("probation_time").value);
			var d = new Date(effect_date);
			d.setMonth(d.getMonth() + probation_time);
		}		
		set_next_increment_date()
	</script>
	<script>
		//To active  menu.......//
		$(document).ready(function() {
			$("#MainGroupEmployee").addClass('active');
			$("#Probation").addClass('active');
		});
	</script>
@endsection