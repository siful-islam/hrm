...
<section class="content">	
	<div class="row">			
		...
		<form class="form-horizontal" action="{{URL::to($action)}}" onsubmit="return change_with_pay()" method="POST" enctype="multipart/form-data">
			{{ csrf_field() }} 
			<input type="hidden" name="db_id" id="db_id"  class="form-control" value="{{$db_id}}">
			<input type="hidden" name="current_open_balance" id="current_open_balance" class="form-control" value="{{$current_open_balance}}" required >
			<input type="hidden"  id="pre_current_close_balance" class="form-control" value="{{$current_close_balance}}" >
			<input type="hidden" name="current_close_balance" id="current_close_balance" class="form-control" value="{{$current_close_balance}}" required >
			<input type="hidden" name="casual_leave_open" id="casual_leave_open" class="form-control" value="{{$casual_leave_open}}"> 
			<input type="hidden" name="pre_casual_leave_close" id="pre_casual_leave_close" class="form-control" value="{{$casual_leave_close}}"> 
			<input type="hidden" name="casual_leave_close" id="casual_leave_close" class="form-control" value="{{$casual_leave_close}}"> 
			
			<input type="hidden" name="emp_id" id="emp_id" class="form-control" value="{{$emp_id}}" required>
			<input type="hidden" name="branch_code" id="branch_code" class="form-control" value="{{$br_code}}" required>
			<input type="hidden" name="designation_code" id="designation_code" class="form-control" value="{{$designation_code}}"> 
			<input type="hidden" name="f_year_id" id="f_year_id" class="form-control" value="{{$f_year_id}}">  
			<input type="hidden" name="org_join_date" id="org_join_date" class="form-control" value="{{$joining_date}}">  
			<input type="hidden" name="tot_earn_leave" id="tot_earn_leave" class="form-control" value="{{$tot_earn_leave}}">
			<input type="hidden" name="pre_cumulative_close" id="pre_cumulative_close" class="form-control" value="{{$pre_cumulative_close}}">  
			<input type="hidden" name="pre_pre_cumulative_close" id="pre_pre_cumulative_close" class="form-control" value="{{$pre_cumulative_close}}">  
			<input type="hidden" name="cum_close_balance" id="cum_close_balance" class="form-control" value="{{$cum_close_balance}}">   
			<input type="hidden" name="pre_cum_close_balance" id="pre_cum_close_balance" class="form-control" value="{{$cum_close_balance}}">   
			<input type="hidden" name="pre_no_of_days" id="pre_no_of_days" class="form-control" value="{{$no_of_days_appr}}">   
			...
			<div class="col-md-8">
						...
						<div class="form-group"> 
									<label class="control-label col-md-2"> Financial Year:</label>
										...
										<div class="col-md-2">
											<input type="text" name="employee_id" id="employee_id" readonly class="form-control" value="{{$emp_id}}"  required>  
						...
						<div class="form-group"> 
									<label class="control-label col-md-3">Serial No</label>
										<div class="col-md-3">
											...
											<input type="hidden" name="f_year_from" id="f_year_from" class="form-control" value="{{$fiscal_year->f_year_from}}" readonly required> 
											<span class="help-block"></span>
										</div> 
									<label class="control-label col-md-3">Application Date </label>
										<div class="col-md-3">
											<input type="text" onchange="calculate_earn_leave()" name="application_date"  id="application_date" class="form-control" value="{{$application_date}}" required>
											<span class="help-block"></span>
									</div> 
						</div> 
						<div class="form-group"> 
					...
					<div class="form-group"> 
									<label class="control-label col-md-3">From </label>
										<div class="col-md-3">
											<input type="text" name="from_date" id="from_date" class="form-control" onchange="settotaldayfrom()" value="{{$from_date}}" required>
											<span class="help-block"></span>
										</div> 
							
									<label class="control-label col-md-3">To </label>
										<div class="col-md-3">
											<input type="text" name="to_date" id="to_date" class="form-control" onchange="settotalday()" value="{{$to_date}}" required>
											<span class="help-block"></span>
										</div> 
								</div> 
							<div class="form-group"> 
									...
									<label class="control-label col-md-3">Remarks</label>
										<div class="col-md-3">
											 <input list="browsers" class="form-control" name="remarks" required value="{{$remarks}}" id="remarks">
											...
											<div class="col-md-3">
												<input type="text" name="no_of_days" id="no_of_days" class="form-control" readonly value="{{$no_of_days_appr}}" required>
												<span class="help-block"></span>
											</div> 										
							</div>
							<div class="form-group"> 
						...
						<div class="form-group"> 
							<label class="control-label col-md-3"></label>
								<div class="col-md-3 col-md-offset-6">
									<a href="{{URL::to('/emp-leave')}}" class="btn bg-olive" >List</a>
			...
			<div class="col-md-4">
				<div class="box box-primary">
					<div class="box-body box-profile">
						...
						<p class="text-muted text-center"><b>Org. Join Date:</b> {{date("d-m-Y",strtotime($joining_date))}}</p>
						 <ul class="list-group list-group-unbordered"><b>( Earn Leave )</b>
								<li class="list-group-item">
								  Total Previous Leave Balance( Cumulative ) <p id="total_previous_leave_balance" class="pull-right">@if(!empty($pre_cumulative_close)){{$pre_cumulative_close}} @else {{0}} @endif</p> 
								</li> 
								<li class="list-group-item">
								 Total Leave this Year( Current ) <p id="this_year_open_balance" class="pull-right">@if(!empty($current_open_balance)){{$current_open_balance}} @else {{0}} @endif</p> 
								</li> 
								<li class="list-group-item">
								 Total Leave enjoy<p  id="total_leave_enjoy" class="pull-right">@if(!empty($no_of_days)){{$no_of_days}} @else {{0}} @endif</p>
								  <input type="hidden" class="form-control" name="tot_expense" id="tot_expense" readonly value="@if(!empty($no_of_days)){{$no_of_days}} @else {{0}} @endif" >
								</li> 
								<li class="list-group-item">
								 Balance<p id="leave_remain1" class="pull-right">{{$leave_remain}}</p>
								  <input type="hidden" name="leave_remain" id="leave_remain" class="form-control" readonly value="{{$leave_remain}}" >
								</li>
						</ul> 
						<ul class="list-group list-group-unbordered"><b>( Casual Leave )</b>
								<li class="list-group-item">
								  Casual Leave Balance <p id="total_casual_leave_balance" class="pull-right"> {{$casual_leave_open}} </p> 
								</li> 
								<li class="list-group-item">
								  Casual Leave enjoy <p  id="total_leave_enjoy_casual" class="pull-right">{{$casual_leave_open - $casual_leave_close  }}  </p> 
								</li> 
								<li class="list-group-item">
								 Casual Leave Balance <p id="leave_remain_casual" class="pull-right"> {{$casual_leave_close}} </p>
...
<script language="javascript">
 ...
 function settotalday1(){
	...
	var emp_id =$("#emp_id").val();  
		$.ajax({
				type:'get',
				url : "{{URL::to('leave_date_exist_edit_br')}}"+"/"+from_date+"/"+to_date+"/"+emp_id+"/"+db_id,
...
$(document).ready(function() {
	 var year = '{{$fiscal_year->f_year_from}}';
