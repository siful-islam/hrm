...
@section('main_content')
					...
					<div class="form-group">
                        ...
                        <div class="col-sm-7">
							<select class="form-control" required name="emp_id" id="emp_id" onchange="set_loan_info();">
								...
								<?php 
								$allowed_designation_group = array(21,16,15,19); 
								?>
								<?php if(in_array($my_staff['designation_group_code'], $allowed_designation_group)) { ?>
    ...
    <script>	
        ...
        $(document).ready(function() {
            table_loan_application = $('#table_loan_application').DataTable({
                ...
                "ajax": {
                    "url": "{{ URL::to('/all_loan_application_bellow_bm') }}",
                    "dataType": "json",
                    "type": "POST",
                    "data": {
                        _token: "{{ csrf_token() }}"
	...
	<script>
		...
		function set_loan_info()
		{
			...
			$.ajax({
				url : "{{ url::to('loan_product_infos') }}"+"/"+loan_type+"/"+emp_id,
				type: "GET",
				dataType: 'JSON',
				success: function(data)
				{
