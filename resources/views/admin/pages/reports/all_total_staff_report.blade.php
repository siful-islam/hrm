...
@section('title', 'All total Staff Report')
	...
	<div class="row">
        <div class="col-md-12"> 
			<div class="box box-info" style="margin-bottom:10px;">
				<div class="box-body">
					<form class="form-inline report" action="{{URL::to('/all-report')}}" method="post">
						{{ csrf_field() }}
						...
						<div class="form-group">
							...
							<select style="width:200px;" class="form-control" id="emp_type" name="emp_type" required >						
								...
								<option value="{{$emptype->id}}">{{$emptype->type_name}}</option>
								@endforeach
							</select>
						</div>
						<div class="form-group">
			...
			<div style="overflow-y: auto; height: 650px;" class="col-xs-12">
				<table id="tblExport" class="table table-bordered" cellspacing="0">
					...
					<tbody>
						...
						<tr>
							<td>{{$i++}}</td>
							<td>{{$result['emp_name_eng']}}<?php if(!empty($result['incharge_as'])) { echo '<span style="color:#F70408"><b> ('.$result['incharge_as'].')</b></span> '; } ?></td>
							<td>{{$result['emp_id']}}</td>
							...
							<td><?php echo ($result['transection'] == 1) ? $result['total_pay'] : round($total_pay);?></td>
							<?php } else { ?>
							<td><?php echo round($result['total_pay']);?></td>
							<?php } ?>
							<td><?php echo $result['net_pay'];?></td>
							<td>{{$result['grade_name']}}</td>
							<td>{{$result['grade_effect_date']}}</td>
							<td><?php 
									$input_date1 = date('Y-m-d', strtotime("+1 day", strtotime($form_date)));
									$input_date = new DateTime($input_date1);
