...
@section('title', 'Designation-Wise Staff Report')
	...
	<div class="row">
        <div class="col-md-12"> 
			<div class="box box-info" style="margin-bottom:10px;">
				<div class="box-body">
					<form class="form-inline report" action="{{URL::to('/designation-staff-report')}}" method="post">
						{{ csrf_field() }}
		...
		<div id="printme">
			...
			<div class="col-md-12">
				...
				<?php } } elseif(($emp_type==1 && $designation_code == 'all' && $order_by == 'emp_id') || ($emp_type==1 && $designation_code != 'all')) { ?>					
					...
					<?php $i=1;
						...
						<tr>
							...
							<?php if ($designation_code == 211 || $designation_code == 212 || $designation_code == 215) { ?>
							<td><?php echo empty($result['open_date']) ? '' : date('d-m-Y', strtotime($result['open_date'])); ?> </td>
							<td>
				...
				<?php } else if ($emp_type != 1) {
					...
					<tbody>
					<?php $i=1; $total_totalpay = 0; foreach($all_result as $result) { $total_totalpay += $result['gross_total']; ?>
						<tr>
							<td>{{$i++}}</td>
							<td><?php echo $result['emp_id']; ?></td>
							<td><?php echo $result['emp_name_eng']; ?></td>
    ...
    $(document).ready(function () {
        $("#btnExport").click(function () {
            $("#tblExport").table2excel({
                exclude:".noExl",
				name:"All Designation List",
				filename:"designation_staff_list",
            });
        });
    });
</script>
