...
<style>
						...
						<table class="table" id="list_approval" style="border">
							...
							<?php if($staf->first_super_action !=2) { ?>
								...
								<tr>
									...
									<td align="center">
										<button type="button" class="btn btn-primary btn-xs" onclick="save_action(<?php echo $staf->move_id; ?>,<?php echo $staf->supervisor_id; ?>,<?php echo $staf->is_need_vehicle_sup; ?>,1);"><i class="fa fa-check" aria-hidden="true"></i> Proceed</button>
										<button type="button" class="btn btn-danger btn-xs" onclick="save_action(<?php echo $staf->move_id; ?>,<?php echo $staf->supervisor_id; ?>,<?php echo $staf->is_need_vehicle_sup; ?>,2);"><i class="fa fa-times" aria-hidden="true"></i> Reject</button>
									</td>
									<?php } elseif ($staf->supervisor_type == 1 && $staf->stage < 2) { ?>
									<td align="center">
										<button type="button" class="btn btn-success btn-xs" onclick="save_action(<?php echo $staf->move_id; ?>,<?php echo $staf->supervisor_id; ?>,<?php echo $staf->is_need_vehicle_sup; ?>,3);"><i class="fa fa-check" aria-hidden="true"></i> Approve</button>
										<button type="button" class="btn btn-danger btn-xs" onclick="save_action(<?php echo $staf->move_id; ?>,<?php echo $staf->supervisor_id; ?>,<?php echo $staf->is_need_vehicle_sup; ?>,4);"><i class="fa fa-times" aria-hidden="true"></i> Reject</button>
									</td>
        ...
        <div class="col-md-12"> 
			<div class="box box-info" style="margin-bottom:10px;">
				<div class="box-body">
					...
					<form class="form-inline report" id="visit_approval_report" action="<?php echo e(URL::to('/approval_report_visit')); ?>" method="post">
						...
						<div class="form-group">
						  ...
						  <input type="date" id="to_date" class="form-control" name="to_date" size="10" value="<?php echo e($to_date); ?>" required>
						</div>
						<div class="form-group">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form class="form-horizontal" role="form" id="v_visit_form"> 
					...
					<input class="form-control" type="hidden" name="is_need_vehicle_sup" id="is_need_vehicle_sup">
			...
			$.ajax({
				...
				success: function (res) {
					...
					var x = document.getElementById("sub_panel");
						...
						document.getElementById("is_need_vehicle_sup").value = res.is_need_vehicle_sup;
	...
	function save_approval()
	{
		...
		url = "<?php echo e(URL::to('/move_appliacation_approve')); ?>";
	...
	function save_action(move_id,supervisor_id,is_need_vehicle_sup,action_id)
	{
		if (confirm('Confirm to execute !!')) {
			var visit_approve_pending = parseInt(document.getElementById("visit_approve_pending").innerHTML);
			var new_visit_approve_pending = (visit_approve_pending - 1);
			var url = "<?php echo e(URL::to('/movement_approval')); ?>";
				$.ajax({
					type: "GET",
					url: url + "/" + move_id+ "/" + supervisor_id+ "/"+ is_need_vehicle_sup+ "/" + action_id,
					success: function (data) {
						$("#list_approval").load(location.href + " #list_approval");
						document.getElementById("visit_approve_pending").innerHTML = new_visit_approve_pending;	
					}
	...
	<script>
		...
		$(document).ready(function() {
				...
				$("#list_approval").load(location.href + " #list_approval");
				$("#visit_approve_pending").load(location.href + " #visit_approve_pending");
			}, 1000);
